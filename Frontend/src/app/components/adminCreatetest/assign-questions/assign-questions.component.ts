...
export class AssignQuestionsComponent implements OnInit {
  ...
  searchTerm: string = '';
  lastOrdre: number = 0;
  @Output() questionsFinalized = new EventEmitter<void>();
  @Output() questionsAssignedEvent = new EventEmitter<void>();
  totalPoints: any;
  ...
  ngOnInit(): void {
    ...
    this.questionService.getAllQuestions().subscribe((data) => {
      ...
      if (this.testId !== null) {
        this.testService.getQuestionsForTest(this.testId).subscribe((assignedQuestions) => {
          console.log('Questions assignées pour ce test:', assignedQuestions);
          this.questions.forEach(question => {
            const assignedQuestion = assignedQuestions.find(q => q.id === question.id);
            if (assignedQuestion) {
              question.isAssigned = true;
  ...
  finalizeQuestions() {
    if (this.testId !== null) {
      this.questionsFinalized.emit();
    }
  }

  assignQuestionToTest(question: Question) {
    if (!question.selectedPoints || question.selectedPoints <= 0) {
      Swal.fire('Erreur', 'Veuillez définir un nombre de points pour cette question avant de l\'assigner.', 'warning');
      return;
    }
    ...
    if (isOrderAlreadyUsed) {
      Swal.fire('Ordre en double', 'Cet ordre est déjà utilisé par une autre question. Veuillez choisir un ordre différent.', 'error');
      return;
    }
    ...
    if (this.testId !== null) {
      ...
      this.testService.addQuestionsToTest(this.testId, [testQuestion], token).subscribe(
        (response) => {
          ...
          Swal.fire('Succès', 'La question a été assignée ou réassignée au test avec succès!', 'success');
        },
        (error) => {
          console.error('Erreur lors de l\'ajout des questions', error);
          Swal.fire('Erreur', 'Une erreur est survenue lors de l\'assignation.', 'error');
        }
      );
    }
  }
